...
/// </summary>
{{~end~}}
public {{x.cs_class_modifier}} partial class {{name}} : {{if parent_def_type}} {{x.parent}} {{else}} Plugin.Bright.Config.BeanBase {{end}}
{
...
{{~end~}}
    ...
    public {{name}}(System.Func<string, ByteBuf> loader)
    {
        ...
        {{~for table in tables ~}}
        {{table.name}} = new {{table.full_name}}(loader(""{{table.output_data_file}}.bin"")); 
